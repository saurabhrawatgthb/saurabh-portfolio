...
export function ExperienceLogSection() {
  return (
    <section id="experience" className="relative min-h-screen w-full px-4 sm:px-8 md:px-16 py-24 bg-black text-white font-sans select-none">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4 border-b-4 border-white pb-4 mb-10">
          ...
          <div className="bg-gta-pink px-4 py-2 text-white font-pricedown text-lg tracking-widest shadow-hard">
        ...
        <div className="space-y-6">
          {experienceLog.map((item, idx) => (
            <div
              key={item.id}
              className="bg-gta-cardDark border-2 border-white/20 p-6 md:p-8 shadow-hard hover:border-gta-yellow transition-all relative group"
            >
              <div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/10 pb-4 mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="bg-gta-yellow text-black font-pricedown text-xs px-2 py-0.5 tracking-wider font-bold">
                      {item.year}
                    </span>
              ...
              <div className="mt-4 flex flex-wrap gap-2 pt-2">
                {item.skillsApplied.map((skill, sIdx) => (
                  <span
                    key={sIdx}
                    className="bg-black border border-white/20 px-2.5 py-1 font-pricedown text-xs tracking-wider text-white/80"
                  >
                    #{skill}
                  </span>
                ))}
